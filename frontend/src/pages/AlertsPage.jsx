    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 8 }}>
        {FILTERS.map(f => (
          <button key={f} onClick={() => setActiveFilter(f)}
            style={{ background: activeFilter === f ? "var(--accent-purple)" : "var(--bg-card)", color: activeFilter === f ? "white" : "var(--text-secondary)", border: "1px solid var(--border)", borderRadius: 8, padding: "6px 16px", fontSize: 13, cursor: "pointer" }}>
            {f}
          </button>
        ))}
      </div>
      ...
      <CardShell title={`Alerts (${sorted.length})`}>
        <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading && <tr><td colSpan={9} style={{ padding: "20px 0", textAlign: "center", color: "var(--text-muted)" }}>Loading...</td></tr>}
            {!loading && sorted.length === 0 && (
              <tr><td colSpan={9} style={{ padding: "20px 0", textAlign: "center", color: "var(--text-muted)" }}>No alerts found</td></tr>
            )}
            {sorted.map(a => (
              <tr key={a.id} style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "10px 0", fontFamily: "monospace", fontSize: 11 }}>{a.mttr_seconds ? `${a.mttr_seconds}s` : "—"}</td>
                <td style={{ padding: "10px 0" }}>
                  <span style={{ ...statusStyle(a.status), padding: "3px 8px", borderRadius: 5, fontSize: 11 }}>{a.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
